 ...
 */
double dist(double *x, double *y, int nDim, int* count = nullptr, double* time = nullptr) {
  struct Dtor {
    Dtor(int *count, double *time) : count(count), time(time) {
      if (time) {
  ...
  if (nDim % 4 != 0) {
    // Fallback to non-SIMD version if nDim is not a multiple of 4
    double sum = 0.0;
...
void computeAssignments(WorkerArgs *const args) {
  ...
  delete[] minDist;
  // args->assignmentTime += CycleTimer::currentSeconds() - start;
}
...
void computeCentroids(WorkerArgs *const args) {
  ...
  delete[] counts;
  // args->centroidTime += CycleTimer::currentSeconds() - start;
}
...
void computeCost(WorkerArgs *const args) {
  ...
  delete[] accum;
  // args->costTime += CycleTimer::currentSeconds() - start;
}
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  double *prevCost = (double*)std::aligned_alloc(32, sizeof(double) * K);
  double *currCost = (double*)std::aligned_alloc(32, sizeof(double) * K);
  double *alignedData = (double*)std::aligned_alloc(32, sizeof(double) * M * N);
  double *alignedCentroids = (double*)std::aligned_alloc(32, sizeof(double) * K * N);
  std::copy(data, data + M * N, alignedData);
  std::copy(clusterCentroids, clusterCentroids + K * N, alignedCentroids);

  // The WorkerArgs array is used to pass inputs to and return output from
  // functions.
  WorkerArgs args;
  args.data = alignedData;
  args.clusterCentroids = alignedCentroids;
  args.clusterAssignments = clusterAssignments;
  args.currCost = currCost;
  args.M = M;
  args.N = N;
  args.K = K;
  // args.assignmentTime = 0.0;
  // args.centroidTime = 0.0;
  // args.costTime = 0.0;
  // args.distCount = 0;
  // args.distTime = 0.0;
  ...
  std::copy(alignedCentroids, alignedCentroids + K * N, clusterCentroids);

  std::free(currCost);
  std::free(prevCost);
  std::free(alignedData);
  std::free(alignedCentroids);

  // printf("Converged in %d iterations.\n", iter);
  // printf("Assignment time: %.3f ms\n", args.assignmentTime * 1000);
  // printf("Centroid time: %.3f ms\n", args.centroidTime * 1000);
